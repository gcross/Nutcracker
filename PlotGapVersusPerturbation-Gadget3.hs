-- @+leo-ver=4-thin
-- @+node:gcross.20091023155936.1509:@thin PlotGapVersusPerturbation-Gadget3.hs
-- @@language Haskell
...
-- @<< Options handling >>
-- @+node:gcross.20091023155936.1510:<< Options handling >>
...
main =
    ...
    flip withSession (do
        ...
        forM number_of_sites_list $ \number_of_sites ->
            let sql_statement = "select perturbation_strength, energy_gap from gadget_model_simulations where model_id = 3 and number_of_sites = " ++ number_of_sites ++ filter_string ++ " order by perturbation_strength desc;"
            in do
    ...
    return . concatMap (\(color,(number_of_sites,data_points)) ->
        [ (PlotStyle LinesPoints (CustomStyle [LineTitle "", LineType color, PointType 2]),data_points)
        ]
    ) . zip [1..]
    >>=
    plotPathsStyle
