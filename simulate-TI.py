...
from utils import crand, normalize_and_return_inverse_normalizer, create_normalized_state_site_tensors, multiply_tensor_by_matrix_at_index
...
def optimize(site_number,next_site_number,normalization_index,move_function):
    ...
    normalized_site_tensor, inverse_normalizer = normalize_and_return_inverse_normalizer(new_site_tensor,normalization_index)

    state_site_tensors[site_number] = normalized_site_tensor
    state_site_tensors[next_site_number] = multiply_tensor_by_matrix_at_index(state_site_tensors[next_site_number],inverse_normalizer.transpose(),3-normalization_index)
...
for sweep_number in xrange(1,number_of_sweeps+1):
    ...
    for site_number in xrange(number_of_sites-1):
        optimize(site_number,site_number+1,2,chain.contract_and_move_right)

    for site_number in xrange(number_of_sites-1,0,-1):
        optimize(site_number,site_number-1,1,chain.contract_and_move_left)
#@-node:gcross.20090930201628.1305:<< Run sweeps >>
#@nl
#@-node:gcross.20090930212235.1299:@thin simulate-TI.py
#@-leo
