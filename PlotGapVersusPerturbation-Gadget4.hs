-- @+leo-ver=4-thin
-- @+node:gcross.20091026125919.1518:@thin PlotGapVersusPerturbation-Gadget4.hs
-- @@language Haskell
...
-- @<< Options handling >>
-- @+node:gcross.20091026125919.1519:<< Options handling >>
...
main =
    ...
    flip withSession (do
        ...
        forM number_of_sites_list $ \number_of_sites ->
            let sql_statement = "select perturbation_strength, energy_gap, infidelity from gadget_model_simulations where number_of_sites = " ++ number_of_sites ++ filter_string ++ " and model_id=4 order by perturbation_strength desc;"
            in do
