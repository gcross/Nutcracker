  ...
  # Class construction methoods {{{
    def __init__(self,sparse_shape,index_to_data_map,nested_shape=None,transposition=None): # {{{
        ...
        if transposition is None:
            ...
            raise ValueError("the length of the transposition is inconsistent with the dimension: len({}) = {} != {}".format(transposition,len(transposition),ndim))
  ...
  # Instance methods {{{
    ...
    def contractWith(self,other,self_sum_axes,other_sum_axes): # {{{
      ...
      # Compute the result shapes {{{
        ...
        print "result_sparse_shape = ",result_sparse_shape
        print "result_nested_shape = ",result_nested_shape
      # }}}
      ...
      # Construct the transposition of the result {{{
        ...
        for ndim, sum_axes, result_axis_to_final_axis_map in [
                (self.ndim,self_sum_axes,self_result_axis_to_final_axis_map),
                (other.ndim,other_sum_axes,other_result_axis_to_final_axis_map),
            ]:
            for axis in xrange(ndim):
                if axis not in sum_axes:
                    result_transposition.append(result_axis_to_final_axis_map[axis])
      # }}}
      # Return the result {{{
