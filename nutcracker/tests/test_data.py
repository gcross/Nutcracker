...
class test_SparseData(TestCase): # {{{
    ...
    def test_contractWith_no_sparse_axes(self,ndimA=irange(0,5),ndimB=irange(0,5)):
    ...
    def test_contractWith_single_sparse_axis(self,
    ...
    def test_contractWith_sparse_only_matrix_multiplication(self,
        ...
        if transpose_result:
            ...
            actual_result = left_sparse_data.contractWith(right_sparse_data,[left_sum_axis],[right_sum_axis]).toArray()
        self.assertAllClose(
            correct_result,
            actual_result,
        )
    ...
    def test_contractWith_no_nested_axes(self,
        ...
        correct_result = correct_result.transpose(
            [axis-number_of_axes_to_sum for axis in left_permutation if axis >= number_of_axes_to_sum]
          + [axis-number_of_axes_to_sum+left_exclusive_ndim for axis in right_permutation if axis >= number_of_axes_to_sum] 
        )
        #left_sparse_data = SparseData(left_shape,left_index_to_data_map,nested_shape=())
        #right_sparse_data = SparseData(right_shape,right_index_to_data_map,nested_shape=())
        #left_sum_axes = range(number_of_axes_to_sum)
        #right_sum_axes = range(number_of_axes_to_sum)
        self.assertAllClose(
            correct_result,
            left_sparse_data.contractWith(right_sparse_data,left_sum_axes,right_sum_axes).toArray()
        )
    # }}}
    @with_checker # test_contractWith_full_case # {{{
    def test_contractWith_full_case(self,
        left_sparse_ndim=irange(0,3),
        right_sparse_ndim=irange(0,3),
        left_nested_ndim=irange(0,3),
        right_nested_ndim=irange(0,3),
        number_of_shared_terms=irange(0,20),
        number_of_left_exclusive_terms=irange(0,20),
        number_of_right_exclusive_terms=irange(0,20),
    ):
        number_of_sparse_axes_to_sum = randint(0,min(left_sparse_ndim,right_sparse_ndim))
        shared_sparse_shape = randomShape(number_of_sparse_axes_to_sum)
        number_of_nested_axes_to_sum = randint(0,min(left_nested_ndim,right_nested_ndim))
        shared_nested_shape = randomShape(number_of_nested_axes_to_sum)
        number_of_axes_to_sum = number_of_sparse_axes_to_sum + number_of_nested_axes_to_sum
        left_ndim = left_sparse_ndim + left_nested_ndim
        left_exclusive_sparse_ndim = left_sparse_ndim-number_of_sparse_axes_to_sum
        left_exclusive_sparse_shape = randomShape(left_exclusive_sparse_ndim)
        left_sparse_shape = shared_sparse_shape + left_exclusive_sparse_shape
        left_exclusive_nested_ndim = left_nested_ndim-number_of_nested_axes_to_sum
        left_exclusive_nested_shape = randomShape(left_exclusive_nested_ndim)
        left_nested_shape = shared_nested_shape + left_exclusive_nested_shape
        left_index_to_data_map = {}
        right_ndim = right_sparse_ndim + right_nested_ndim
        right_exclusive_sparse_ndim = right_sparse_ndim-number_of_sparse_axes_to_sum
        right_exclusive_sparse_shape = randomShape(right_exclusive_sparse_ndim)
        right_sparse_shape = shared_sparse_shape + right_exclusive_sparse_shape
        right_exclusive_nested_ndim = right_nested_ndim-number_of_nested_axes_to_sum
        right_exclusive_nested_shape = randomShape(right_exclusive_nested_ndim)
        right_nested_shape = shared_nested_shape + right_exclusive_nested_shape
        right_index_to_data_map = {}
        generated_shared_sparse_indices = set()
        shared_sparse_indices = None
        generated_shared_sparse_indices.add(shared_sparse_indices)
        for _ in xrange(number_of_shared_terms):
            shared_sparse_indices = randomIndices(shared_sparse_shape)
            if shared_sparse_indices in generated_shared_sparse_indices:
                continue
            generated_shared_sparse_indices.add(shared_sparse_indices)
            left_index_to_data_map[shared_sparse_indices + randomIndices(left_exclusive_sparse_shape)] = NDArrayData.newRandom(left_nested_shape)
            right_index_to_data_map[shared_sparse_indices + randomIndices(right_exclusive_sparse_shape)] = NDArrayData.newRandom(right_nested_shape)
        for _ in xrange(number_of_left_exclusive_terms):
            shared_sparse_indices = randomIndices(shared_sparse_shape)
            if shared_sparse_indices in generated_shared_sparse_indices:
                continue
        ...
        for _ in xrange(number_of_right_exclusive_terms):
            ...
            generated_shared_sparse_indices.add(shared_sparse_indices)
            right_index_to_data_map[shared_sparse_indices + randomIndices(right_exclusive_sparse_shape)] = NDArrayData.newRandom(right_nested_shape)
        left_sparse_permutation = randomPermutation(left_sparse_ndim)
        left_index_to_data_map = {tuple(applyPermutation(left_sparse_permutation,indices)): data for (indices,data) in left_index_to_data_map.iteritems()}
        left_inverse_sparse_permutation = invertPermutation(left_sparse_permutation)
        left_sparse_sum_axes = left_inverse_sparse_permutation[:number_of_sparse_axes_to_sum]
        left_sparse_shape = tuple(applyPermutation(left_sparse_permutation,left_sparse_shape))
        left_nested_sum_axes = range(left_sparse_ndim,left_sparse_ndim+number_of_nested_axes_to_sum)
        left_transposition = randomPermutation(left_ndim)
        left_sum_axes = [left_transposition.index(axis) for axis in chain(left_sparse_sum_axes,left_nested_sum_axes)]
        left_sparse_data = SparseData(left_sparse_shape,left_index_to_data_map,nested_shape=left_nested_shape).transpose(left_transposition)
        left_ndarray_data = left_sparse_data.toNDArrayData()
        right_sparse_permutation = randomPermutation(right_sparse_ndim)
        right_index_to_data_map = {tuple(applyPermutation(right_sparse_permutation,indices)): data for (indices,data) in right_index_to_data_map.iteritems()}
        right_inverse_sparse_permutation = invertPermutation(right_sparse_permutation)
        right_sparse_sum_axes = right_inverse_sparse_permutation[:number_of_sparse_axes_to_sum]
        right_sparse_shape = tuple(applyPermutation(right_sparse_permutation,right_sparse_shape))
        right_nested_sum_axes = range(right_sparse_ndim,right_sparse_ndim+number_of_nested_axes_to_sum)
        right_transposition = randomPermutation(right_ndim)
        right_sum_axes = [right_transposition.index(axis) for axis in chain(right_sparse_sum_axes,right_nested_sum_axes)]
        right_sparse_data = SparseData(right_sparse_shape,right_index_to_data_map,nested_shape=right_nested_shape).transpose(right_transposition)
        right_ndarray_data = right_sparse_data.toNDArrayData()
        sum_axes_permutation = randomPermutation(number_of_axes_to_sum)
        left_sum_axes = applyPermutation(sum_axes_permutation,left_sum_axes)
        right_sum_axes = applyPermutation(sum_axes_permutation,right_sum_axes)
        self.assertAllClose(
            left_ndarray_data.contractWith(right_ndarray_data,left_sum_axes,right_sum_axes).toArray(),
            left_sparse_data.contractWith(right_sparse_data,left_sum_axes,right_sum_axes).toArray(),
        )
